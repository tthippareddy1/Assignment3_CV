...
// ---------- Task 2: Corner – Harris with Shi–Tomasi fallback ----------
function cornersHarris(src) {
  try {
    // (this is your robust Harris from earlier)
    const work = maybeResize(src);
    const gray = toGray(work);
    cv.GaussianBlur(gray, gray, new cv.Size(3,3), 0.8, 0.8);
...
function boundaryContour(src) {
  ...
  // 1) Thresholds for Canny: auto (Otsu on grad mag) or from sliders
  let low = 20, high = 60;
  if (!els.edgeAuto || els.edgeAuto.checked) {
    const mag8 = gradMagnitudeMat(work);  // 8U
    const tmp = new cv.Mat();
    const otsu = cv.threshold(mag8, tmp, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU); // returns threshold
  ...
  cv.Canny(gray, edges, low, high, 3, true);

  // 3) Morphological closing to bridge gaps
  ...
  // center bias: keep contours whose centroid is within R% of diagonal from center
  const centerR = (els.bCenterR ? +els.bCenterR.value : 40) / 100; // 0..0.8 of diag
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    const dist = Math.hypot(cx - cx0, cy - cy0) / diag; // 0..~0.7
    const centerOK = dist <= centerR;
    // score: prefer bigger, and prefer closer to center
  ...
  if (bestIdx < 0) {
    contours.delete();
    overlayModeLabel('BOUNDARY – NONE');
    work.delete();
    return { out, info: 'no contour' };
  }

  // 5) Refine best contour with approxPolyDP and build a mask
  ...
  // 6) Show either binary mask or overlay
  if (els.edgeBinary && els.edgeBinary.checked) {
    ...
    fillRGB.copyTo(filled, mask);

    const blended = new cv.Mat();
    cv.addWeighted(out, 0.65, filled, 0.35, 0, blended);
  ...
  if (state.source === 'dataset' && state.images.length) {
    ...
    try {
      if (mode === 'original') {
        ...
        out = E; imshowFit(out); overlayModeLabel('EDGE – NMS + HYST');
        stats = ` | edges: ${count}`;
      } else if (mode === 'corners_harris') {
        const { out: C, count } = cornersHarris(src);
        out = C; imshowFit(out); overlayModeLabel('CORNERS – HARRIS');
        stats = ` | corners: ${count}`;
      } else if (mode === 'boundary_contour') {
        const { out: B, info } = boundaryContour(src);
        out = B; imshowFit(out); overlayModeLabel('BOUNDARY – CONTOUR');
        stats = info ? ` | ${info}` : '';
      } else {
        drawBitmapToCanvas(bmp); overlayModeLabel('ORIGINAL');
      }
...
els.fileInput.onchange = async (e) => {
  try {
    ...
    console.error(err);
    setStatus('Failed to load images. Try JPG/PNG (not HEIC).');
  }
};
...
['edgeBoost','halfRes','edgeLow','edgeHigh','edgeAuto','edgeBinary','hWin','hK','hTh','hHeat','bClose','bMinArea','bEps','bCenterR']
...
els.exportAll2.onclick = async () => {
  ...
  for (let i = 0; i < state.images.length; i++) {
    ...
    try {
      const { out: E } = edgeSimple(srcRGBA, true);
      saveMatAsPNG(E, `${stem}__edge.png`); E.delete();

      const { out: C } = cornersHarris(srcRGBA);
      saveMatAsPNG(C, `${stem}__corners.png`); C.delete();
    } catch (e) {
